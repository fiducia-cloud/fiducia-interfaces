...
create table if not exists operations_runners (
  ...
  updated_at timestamptz default now() not null,
  version bigint default 1 not null,
  constraint operations_runners_status_chk check (status in ('active','online','busy','draining','offline','revoked')),
...
create table if not exists operation_executions (
  ...
  updated_at timestamptz default now() not null,
  version bigint default 1 not null,
  constraint operation_executions_status_chk check (status in ('assigned','running','succeeded','failed','cancelled','timed_out','cleaning','cleanup_pending','cleaned')),
...
create index if not exists operation_executions_operation_idx on operation_executions (operation_id, operation_generation, batch_number, attempt);
...
create table if not exists resource_observations (
  ...
  constraint resource_observations_actual_object_chk check (jsonb_typeof(actual_state) = 'object')
);
...
create index if not exists operations_audit_operation_created_idx on operation_audit (operation_id, created_at desc);
